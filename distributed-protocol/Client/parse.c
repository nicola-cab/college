...
//massimo numero di contenuti che posso leggere dal server (da migliorare come gestione)
...
int split_mess(char mess[] ,char ** values){
	...
	logger("Eseguo split della response");
	...
	do{
		...
		//copio nel vettore di stringhe 
			...
			//printf("%d\n",flag);
			//compose_mess_to_log("stringa da copiare: ",buff,STRING_TYPE);
			values[num_stringhe-1] = (char *)malloc(strlen(mess)+1); //alloco la memoria
			strcpy(values[num_stringhe-1],buff);	//copio la stringa
			//compose_mess_to_log("stringa da copiare: ",values[num_stringhe-1],STRING_TYPE);	
